...
prgm = select(datstu, choicepgm1, choicepgm2, choicepgm3, choicepgm4, choicepgm5, choicepgm6, X)
prgm_Total = gather(prgm, program, progname, choicepgm1, choicepgm2, choicepgm3, choicepgm4, choicepgm5, choicepgm6)
prgm_Total_Unique = unique(prgm_Total)
cat('1.3 The number of program is:', nrow(prgm_Total_Unique), '\n')

#4 Number of choices (school,program)
school = datstu %>% select(schoolcode1, schoolcode2, schoolcode3, schoolcode4, schoolcode5, schoolcode6, X) %>% gather (, schoolcode, schoolcode1, schoolcode2, schoolcode3, schoolcode4, schoolcode5, schoolcode6)
choices = cbind(school[,3], prgm_Total[,3])
...
dataselected <- bind_rows(a,b,c,d,e,f)
numberofchoices <- unique(dataselected)
dataselected <- [!(is.na(numberofchoices$schoolcode)),]
...
datanew<-merge(x=numberofchoices,y=datsss2,by="schoolcode",all.x = TRUE, all.y = FALSE)
...
data3$quantile <- smart_cut(datanew2$score, 4, "g", output = "numeric")
data3$quantile <- replace(datanew2$quantile, datanew2$quantile==1, "0%-25%")
data3$quantile <- replace(datanew2$quantile, datanew2$quantile==2, "25%-50%")
data3$quantile <- replace(datanew2$quantile, datanew2$quantile==3, "50%-75%")
data3$quantile <- replace(datanew2$quantile, datanew2$quantile==4, "75%-100%")
